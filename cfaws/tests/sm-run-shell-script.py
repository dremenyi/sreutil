...
def get_command_output(instance_ids, command_id, region='us-east-1', max_retries=60, delay=10):
    ssm_client = boto3.client('ssm', region_name=region)
    output = {}
...
def main():
    parser = argparse.ArgumentParser(description='Run a shell script on EC2 instances using AWS Systems Manager.')
    parser.add_argument('--targets', nargs='*', help='List of instance names to target.')
    parser.add_argument('--all-linux-instances', action='store_true', help='Target all running Linux instances.')
    parser.add_argument('--file', type=str, required=True, help='Path to the shell script file to execute.')

    args = parser.parse_args()

    if not args.all_linux_instances and not args.targets:
        parser.error('You must specify --all-linux-instances or provide instance names with --targets.')
